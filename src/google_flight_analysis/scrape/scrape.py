    ...
    def _extract_flights(self, driver: ChromeDriver) -> List[ScrapeResult]:
        ...
        try:
            ...
            flight_cards = driver.find_elements(
                "xpath",
                "//li[contains(@class, 'pIav2d')] | //div[contains(@class, 'flight-card')]"
            )
        ...
        try:
            ...
            try:
                price_elem = card.find_element(
                    "xpath",
                    ".//span[contains(text(), '$')] | .//div[contains(@class, 'price')]"
                )
            ...
            try:
                duration_elem = card.find_element(
                    "xpath",
    ...
    def save_to_db(self, db_path: Optional[str] = None) -> int:
        ...
        for result in self._results:
            dep_dt = datetime.strptime(
                f"{result.departure_date} {result.departure_time}",
                "%Y-%m-%d %H:%M"
            )
            arr_dt = None
            if result.arrival_time:
                try:
                    arr_dt = datetime.strptime(
                        f"{result.departure_date} {result.arrival_time}",
                        "%Y-%m-%d %H:%M"
                    )
                except:
                    pass
